...
export default async function TeamPage() {
  // Server-side subscription verification (cannot be bypassed by client)
  const { has } = await auth();

  // Check subscription status server-side first
  const hasPaidPlan = has ? !has({ plan: "free_user" }) : false;

  // If server-side check fails, show upgrade card
  if (!hasPaidPlan) {
    return <UpgradeCard />;
  }

  // Double-layer protection: Server-side check passed, client Protect provides UX consistency
  return (
    <Protect
