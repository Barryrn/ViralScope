...
export default async function FeaturesOne() {
            ...
            <div className=" py-24">
                <div className="mx-auto w-full max-w-5xl px-6">
                    <div className="text-center">
                        ...
                        <p className="text-muted-foreground mb-12 mt-4 text-balance text-lg">{t('description')}</p>
                    ...
                    <div className="border-foreground/10 relative mt-16 grid gap-12 border-b pb-12 [--radius:1rem] md:grid-cols-2">
                        <div>
                            <h3 className="text-foreground text-xl font-semibold">{t('marketing.title')}</h3>
                            <p className="text-muted-foreground my-4 text-lg">{t('marketing.description')}</p>
                            <Card
                        ...
                        <div>
                            <h3 className="text-foreground text-xl font-semibold">{t('scheduler.title')}</h3>
                            <p className="text-muted-foreground my-4 text-lg">{t('scheduler.description')}</p>
                            <Card
                    ...
                    <blockquote className="before:bg-primary relative mt-12 max-w-xl pl-6 before:absolute before:inset-y-0 before:left-0 before:w-1 before:rounded-full">
                        <p className="text-foreground text-lg">{t('testimonialQuote')}</p>
                        <footer className="mt-4 flex items-center gap-2">
                            <cite>{t('testimonialAuthor')}</cite>
                            <span
                                aria-hidden
                                className="bg-foreground/15 size-1 rounded-full"></span>
                            <span className="text-muted-foreground">{t('testimonialRole')}</span>
                        </footer>
                    </blockquote>
                </div>
            </div>
